...
def train(params, output_dir):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (rgb_images, thermal_images, labels, rgb_dirs, thermal_dirs) in enumerate(train_loader):
            ...
            # print (rgb_dirs, thermal_dirs, labels)
            print(f'Batch {batch_idx + 1}/{num_batches}, Train Loss: {x}')
        ...
        val_loss = 0.0
        with torch.no_grad():
            for val_batch_idx, (rgb_images, thermal_images, labels, rgb_dirs, thermal_dirs) in enumerate(val_loader):
                rgb_images = rgb_images.to(device)
                thermal_images = thermal_images.to(device)
                labels = labels.to(device)
                outputs, _, _, _ = model(rgb_images, thermal_images)
        ...
        print(f"Epoch {epoch + 1}/{num_epochs}, Train Loss: {train_loss}, Validation Loss: {val_loss}, Epoch Time: {epoch_time:.2f} minutes, Learning rate: {optimizer.param_groups[0]['lr']}", flush=True)
    ...
    test_loop(model, test_loader, device, output_dir, load_checkpoint=False)
